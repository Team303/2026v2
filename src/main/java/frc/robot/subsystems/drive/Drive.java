...
public class Drive extends SubsystemBase {
  ...
  public Pose2d getFuelPose2dPath() {
    ...
    Rotation2d rotation =
        new Rotation2d(getPose().getRotation().getRadians() - (xAngle * (Math.PI / 180)));

    return new Pose2d(
        new Translation2d(
                getPose().getTranslation().getX() /*meters*/, getPose().getTranslation().getY())
            .plus(
                new Translation2d(-1.5, 0).rotateBy(Rotation2d.fromDegrees(rotation.getDegrees()))),
        rotation);
  }
